...
export const getParticipantsData = query({
  ...
  handler: async (ctx, args) => {
    ...
    return Promise.all(
      (event?.participants ?? []).map((userId) => ctx.db.get(userId))
    );
  },
});
...
export const syncPalate = action({
    ...
    handler: async (ctx, args) => {
        ...
        let recipes = [];
        for (let i = 0; i < 3; i++) {
          const dishes = await ctx.runAction(api.dishes.searchDishes, {ingredients: ingredients,
            dislikedIngredients: dislikedIngredients, cuisine: cuisines});
